...
for n=1:numChirps+2*N_g 
    for m=1:numADCsamples+2*N_g
        ...
        image(n,m)=dataused_rv(n-N_g+numChirps*temp1,m-N_g+numADCsamples*temp2);
    end
end
...
basket = zeros(1, ((2*N_g+1)^2));
k = 100; % rank number
S_ca = 40;

for v_dir = 1:numChirps % 1~128
    for r_dir = 1:numADCsamples %1~256
        for temp_a = 0:2*N_g % 0~20
            for temp_b = 0:2*N_g % 0~20
                basket(1,(temp_a*(2*N_g+1))+temp_b+1) = image(v_dir+temp_a, r_dir+temp_b);
            end
        end

        for temp_a = 0:2*guard
            basket(1, (2*N_g+1)*(N_g-guard)+N_g-guard+1+2*temp_a*(N_g-guard):(2*N_g+1)*(N_g-guard)+N_g+guard+1+2*temp_a*(N_g-guard)) = 0;
        end
        sorted_data = sort(abs(basket));
        Threshold2(v_dir, r_dir) = sorted_data(k+(2*guard+1)^2)*S_ca;
